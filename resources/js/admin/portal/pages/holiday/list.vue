<template>
    ...
    <div class="p-3 bg-white w-full block rounded-lg">
        ...
        <div class="w-full flex justify-between items-center mb-3">
            ...
            <router-link :to="{name:'manageHoliday',state:{id:null}}" class="cursor-pointer min-w-[90px] max-w-[90px] min-h-[45px] max-h-[45px] duration-500 bg-blue-500 hover:bg-blue-700 text-white inline-flex justify-center items-center rounded-lg">
                Add
            </router-link>
        </div>
            ...
            <div class="overflow-x-auto w-full block">
                <table class="w-full table block">
                    ...
                    <tr class="w-full">
                        ...
                        <th class="min-w-[300px] max-w-[300px] font-medium text-gray-900 text-start truncate px-4 py-3 !text-[14px]">
                            Details
                        </th>
                        <th class="min-w-[150px] max-w-[150px] font-medium text-gray-900 text-start truncate px-4 py-3 !text-[14px]">
                            Action
                    ...
                    <tr class="w-full border border-gray-200" v-for="(each, index) in tableData" :key="index">
                        ...
                        <td class="min-w-[150px] max-w-[150px] font-normal text-start truncate px-4 py-3 !text-[14px] text-gray-600">
                            <template v-if="each.type === 'republic_day'"> Republic Day </template>
                            <template v-if="each.type === 'independence_day'"> Independence Day </template>
                            <template v-if="each.type === 'memorial_day'"> Memorial Day </template>
                        </td>
                        <td class="min-w-[150px] max-w-[150px] font-normal text-start truncate px-4 py-3 !text-[14px] text-gray-600">
                            {{each.start_date_formatted}}
                        </td>
                        <td class="min-w-[150px] max-w-[150px] font-normal text-start truncate px-4 py-3 !text-[14px] text-gray-600">
                            {{each.end_date_formatted}}
                        </td>
                        <td class="min-w-[300px] max-w-[300px] font-normal text-start truncate px-4 py-3 !text-[14px] text-gray-600">
                            {{each.details}}
                        </td>
                        <td class="min-w-[150px] max-w-[150px] font-normal text-start truncate px-4 py-3 !text-[14px] text-gray-600">
                            <div class="flex justify-start items-center gap-x-2">
                                <router-link :to="{name:'manageHoliday',state:{id:each.id}}" class="cursor-pointer duration-500 inline-flex justify-center items-center max-w-[35px] min-w-[35px] max-h-[35px] min-h-[35px] rounded-lg bg-gray-100 text-gray-900 hover:bg-gray-300">
...
export default {
    ...
    methods: {
        ...
        async readApi(page = 1) {
            try {
                this.loading = true;
                const response = await axios.get(`${apiRoute.admin.holiday}`, {
                    params: {
                        page,
        ...
        async deleteApi() {
            try {
                ...
                await axios.delete(apiRoute.admin.holiday+`/delete/${this.id}`, {headers: apiService.authHeaderContent()});
