        ...
        <div class="w-full flex justify-between items-center mb-3">
            <div class="w-full max-w-[450px]">
                <div class="relative">
                    ...
                    <input type="text" name="search" placeholder="Search Here ..." class="ps-10 pe-4 w-full min-h-[45px] max-h-[45px] block px-4 outline-0 border border-gray-200 placeholder-gray-900 text-[14px] ring-0 ring-transparent focus-within:ring-2 focus-within:ring-blue-400 rounded-lg duration-500" />
                </div>
            </div>
            <router-link :to="{name:'manageStudent',state:{id:null}}" class="!text-[14px] cursor-pointer min-w-[90px] max-w-[90px] min-h-[45px] max-h-[45px] duration-500 bg-blue-500 hover:bg-blue-700 text-white inline-flex justify-center items-center rounded-lg">
                Add
            </router-link>
        </div>
        <div class="min-h-[calc(100vh-320px)] max-h-[calc(100vh-320px)] overflow-y-auto" v-if="!loading & tableData.length > 0">
            <div class="overflow-x-auto w-full block">
                <table class="w-full table block">
                    ...
                    <tr class="w-full">
                        <th class="min-w-[290px] max-w-[290px] font-medium text-gray-900 text-start truncate px-4 py-3 !text-[14px]">
                            Name
                        </th>
                        <th class="min-w-[290px] max-w-[290px] font-medium text-gray-900 text-start truncate px-4 py-3 !text-[14px]">
                            Email
                        </th>
                        <th class="min-w-[200px] max-w-[200px] font-medium text-gray-900 text-start truncate px-4 py-3 !text-[14px]">
                            Phone
                        </th>
                        <th class="min-w-[200px] max-w-[200px] font-medium text-gray-900 text-start truncate px-4 py-3 !text-[14px]">
                            Enrolled Course
                        ...
                        <th class="min-w-[170px] max-w-[170px] font-medium text-gray-900 text-start truncate px-4 py-3 !text-[14px]">
                            Registration Date
                        </th>
                        <th class="min-w-[170px] max-w-[170px] font-medium text-gray-900 text-start truncate px-4 py-3 !text-[14px]">
                            Action
                    ...
                    <tr class="w-full border border-gray-200" v-for="(each, index) in tableData" :key="index">
                        <td class="min-w-[290px] max-w-[290px] font-normal text-start truncate px-4 py-3 !text-[14px] text-gray-600">
                            {{each.name || 'N/A'}}
                        </td>
                        <td class="min-w-[290px] max-w-[290px] font-normal text-start truncate px-4 py-3 !text-[14px] text-gray-600">
                            {{each.email || 'N/A'}}
                        </td>
                        <td class="min-w-[200px] max-w-[200px] font-normal text-start truncate px-4 py-3 !text-[14px] text-gray-600">
                            {{each.mobile_number || 'N/A'}}
                        </td>
                        <td class="min-w-[200px] max-w-[200px] font-normal text-start truncate px-4 py-3 !text-[14px] text-gray-600">
                            {{each.enrolled_course || 'N/A'}}
                        </td>
                        <td class="min-w-[200px] max-w-[200px] font-normal text-start truncate px-4 py-3 !text-[14px] text-gray-600">
                            {{each.blood_group || 'N/A'}}
                        </td>
                        <td class="min-w-[170px] max-w-[170px] font-normal text-start truncate px-4 py-3 !text-[14px] text-gray-600">
                            {{each.registration_date_formatted || 'N/A'}}
                        </td>
                        <td class="min-w-[170px] max-w-[170px] font-normal text-start truncate px-4 py-3 !text-[14px] text-gray-600">
                            <div class="flex justify-start items-center gap-x-2">
                                <router-link :to="{name:'manageStudent',state:{id:each.id}}" class="cursor-pointer duration-500 inline-flex justify-center items-center max-w-[35px] min-w-[35px] max-h-[35px] min-h-[35px] rounded-lg bg-gray-100 text-gray-900 hover:bg-gray-300">
...
export default {
    ...
    methods: {
        ...
        async readApi(page = 1) {
            try {
                this.loading = true;
                const response = await axios.get(`${apiRoute.admin.student}`, {
                    params: {
                        page,
        ...
        async deleteApi() {
            try {
                ...
                await axios.delete(apiRoute.admin.student+`/delete/${this.id}`, {headers: apiService.authHeaderContent()});
