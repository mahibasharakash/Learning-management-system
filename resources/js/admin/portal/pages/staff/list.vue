<template>
    ...
    <div class="p-3 bg-white w-full block rounded-lg">
        ...
        <div class="w-full flex justify-between items-center mb-3">
            ...
            <router-link :to="{name:'manageStaff',state:{id:null}}" class="cursor-pointer min-w-[90px] max-w-[90px] min-h-[45px] max-h-[45px] duration-500 bg-blue-500 hover:bg-blue-700 text-white inline-flex justify-center items-center rounded-lg">
                Add
            </router-link>
        </div>
            ...
            <div class="overflow-x-auto w-full block">
                <table class="w-full table block">
                    ...
                    <tr class="w-full border border-gray-200" v-for="(each, index) in tableData" :key="index">
                        ...
                        <td class="min-w-[170px] max-w-[170px] font-normal text-start truncate px-4 py-3 !text-[14px] text-gray-600">
                            {{each.joining_date_formatted}}
                        </td>
                        <td class="min-w-[170px] max-w-[170px] font-normal text-start truncate px-4 py-3 !text-[14px] text-gray-600">
                            <div class="flex justify-start items-center gap-x-2">
                                <router-link :to="{name:'manageStaff',state:{id:each.id}}" class="cursor-pointer duration-500 inline-flex justify-center items-center max-w-[35px] min-w-[35px] max-h-[35px] min-h-[35px] rounded-lg bg-gray-100 text-gray-900 hover:bg-gray-300">
                                    <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="1.5" stroke="currentColor" class="w-5 h-5">
                                        <path stroke-linecap="round" stroke-linejoin="round" d="m16.862 4.487 1.687-1.688a1.875 1.875 0 1 1 2.652 2.652L10.582 16.07a4.5 4.5 0 0 1-1.897 1.13L6 18l.8-2.685a4.5 4.5 0 0 1 1.13-1.897l8.932-8.931Zm0 0L19.5 7.125M18 14v4.75A2.25 2.25 0 0 1 15.75 21H5.25A2.25 2.25 0 0 1 3 18.75V8.25A2.25 2.25 0 0 1 5.25 6H10" />
                                    </svg>
                                </router-link>
                                <button type="button" class="cursor-pointer duration-500 inline-flex justify-center items-center max-w-[35px] min-w-[35px] max-h-[35px] min-h-[35px] rounded-lg bg-red-100 text-red-900 hover:bg-red-500 hover:text-white">
                                    <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="1.5" stroke="currentColor" class="w-5 h-5">
...
export default {
    ...
    methods: {
        ...
        async readApi(page = 1) {
            try {
                this.loading = true;
                const response = await axios.get(`${apiRoute.admin.staff}`, {
                    params: {
                        page,
        ...
        async deleteApi() {
            try {
                ...
                await axios.delete(apiRoute.admin.staff+`/delete/${this.id}`, {headers: apiService.authHeaderContent()});
