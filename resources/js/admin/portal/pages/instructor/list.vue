<template>
    ...
    <div class="p-3 bg-white w-full block rounded-lg">
        ...
        <div class="w-full flex justify-between items-center mb-3">
            ...
            <router-link :to="{name:'manageInstructor',state:{id:null}}" class="!text-[14px] cursor-pointer min-w-[90px] max-w-[90px] min-h-[45px] max-h-[45px] duration-500 bg-blue-500 hover:bg-blue-700 text-white inline-flex justify-center items-center rounded-lg">
                Add
            </router-link>
        </div>
            ...
            <div class="overflow-x-auto w-full block">
                <table class="w-full table block">
                    ...
                    <tr class="w-full border border-gray-200" v-for="(each, index) in tableData" :key="index">
                        ...
                        <td class="min-w-[150px] max-w-[150px] font-normal text-start truncate px-4 py-3 !text-[14px] text-gray-600">
                            {{each.date_of_birth_formatted || 'N/A'}}
                        </td>
                        <td class="min-w-[200px] max-w-[200px] font-normal text-start truncate px-4 py-3 !text-[14px] text-gray-600">
                            {{each.educational_qualification || 'N/A'}}
                        </td>
                        <td class="min-w-[150px] max-w-[150px] font-normal text-start truncate px-4 py-3 !text-[14px] text-gray-600">
                            <div class="flex justify-start items-center gap-x-2">
                                <router-link :to="{name:'manageInstructor',state:{id:each.id}}" class="cursor-pointer duration-500 inline-flex justify-center items-center max-w-[35px] min-w-[35px] max-h-[35px] min-h-[35px] rounded-lg bg-gray-100 text-gray-900 hover:bg-gray-300">
...
export default {
    ...
    methods: {
        ...
        async readApi(page = 1) {
            try {
                this.loading = true;
                const response = await axios.get(`${apiRoute.admin.tutor}`, {
                    params: {
                        page,
