    ...
    <form @submit.prevent="manageApi" class="bg-white px-5 py-3 rounded-lg">
        <div class="w-full grid grid-cols-1 md:grid-cols-2 xl:grid-cols-3 gap-5">
            ...
            <div class="w-full block">
                ...
                <div class="text-red-500 mt-1 w-full block text-[12px] font-medium" v-if="error.joining_date"> {{error.joining_date[0]}} </div>
            </div>
            <div class="w-full block">
            ...
            <div class="w-full block">
                ...
                <div class="text-red-500 mt-1 w-full block text-[12px] font-medium" v-if="error.designation"> {{error.designation[0]}} </div>
            </div>
            <div class="w-full block">
            ...
            <div class="w-full block">
                ...
                <div class="text-red-500 mt-1 w-full block text-[12px] font-medium" v-if="error.educational_qualification"> {{error.educational_qualification[0]}} </div>
            </div>
            <div class="w-full block">
            ...
            <div class="w-full block md:col-span-2 xl:col-span-3">
                <div class="w-full flex justify-end items-center gap-3">
                    <router-link :to="{name:'instructors'}" class="min-w-[120px] max-w-[120px] min-h-[45px] max-h-[45px] inline-flex justify-center items-center bg-gray-100 duration-500 hover:bg-gray-300 text-gray-900 rounded-lg text-xs font-medium">
                        Cancel
                    </router-link>
    ...
    methods: {
        ...
        async createApi() {
            try {
                ...
                await axios.post(apiRoute.admin.tutor + '/store', this.buildFormData(), {
        ...
        async updateApi(id) {
            try {
                this.manageLoading = true;
                await axios.post(apiRoute.admin.tutor + `/update/${id}?_method=PUT`, this.buildFormData(), {
                    headers: apiService.authHeaderMediaContent()
                });
                this.$router.push({ name: 'instructors' });
            } catch (e) {
        ...
        async singleApi(id) {
            try {
                ...
                const { data } = await axios.get(apiRoute.admin.tutor+`/show/${id}`, { headers: apiService.authHeaderMediaContent() });
